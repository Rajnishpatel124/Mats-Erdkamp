...
const AnimatedText3D = () => {
  ...
  const [pointsGeom, setPointsGeom] = useState<THREE.BufferGeometry | null>(null);
  ...
  useEffect(() => {
    if (meshRef.current) {
      // Small timeout to allow TextGeometry to be generated
      setTimeout(() => {
  ...
  useFrame((state) => {
    ...
    // Animation: Wait 0.5s, then morph over 2 seconds
    const t = Math.max(0, Math.min((elapsed - 0.5) / 2.0, 1));
    
    if (pointsMatRef.current) {
      pointsMatRef.current.opacity = 1 - t;
...
const BackgroundText = () => {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center opacity-[0.04] select-none pointer-events-none overflow-hidden space-y-[-5vw] z-0">
      <motion.h1 
        style={{ x: xLeft }}
        className="text-[25vw] font-display font-bold uppercase tracking-tighter whitespace-nowrap"
      >
        RAJNISH PATEL RAJNISH PATEL RAJNISH PATEL
      </motion.h1>
      <motion.h1 
        style={{ x: xRight }}
        className="text-[25vw] font-display font-bold uppercase tracking-tighter whitespace-nowrap"
